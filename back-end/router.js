...
ROUTER.route('/badgeclasses/').get(BADGECLASSES.get).post(BADGECLASSES.post).options(OPTIONS.collectionOptions);
ROUTER.route('/badgeclasses/:id').get(BADGECLASSES.get).patch(BADGECLASSES.update).put(BADGECLASSES.rewrite).delete(BADGECLASSES.delete).options(OPTIONS.detailOptions);
